...
fn part2(input: &str) -> Result<()> {
    ...
    for a in &actions {
        mov2(a, &mut heading, &mut ship, &mut waypoint);
    }
...
fn mov2(a: &Action, heading: &mut Action, ship: &mut Point, waypoint: &mut Point) {
    match a {
        Action::N(v) => move_dir2(&mut Action::N(0), waypoint, *v),
        Action::S(v) => move_dir2(&mut Action::S(0), waypoint, *v),
        Action::E(v) => move_dir2(&mut Action::E(0), waypoint, *v),
        Action::W(v) => move_dir2(&mut Action::W(0), waypoint, *v),
        Action::L(_) => {
            turn2(a, heading, waypoint);
        }
        Action::R(_) => {
            turn2(a, heading, waypoint);
        }
        Action::F(v) => {
            move_ship(heading, ship, waypoint, *v);
        }
    }
}
...
fn turn2(a: &Action, heading: &mut Action, waypoint: &mut Point) {
    let current = match heading {
        Action::N(_) => 1,
        Action::S(_) => 3,
        Action::E(_) => 0,
...
fn move_dir(heading: &mut Action, ship: &mut Point, v: i32) {
    match heading {
        Action::N(_) => ship.y += v,
        // Action::S(_) => ship.y = (ship.y - v).abs(),
        Action::S(_) => ship.y -= v,
        Action::E(_) => ship.x += v,
        // Action::W(_) => ship.x = (ship.x - v).abs(),
        Action::W(_) => ship.x -= v,
...
fn move_ship(heading: &mut Action, ship: &mut Point, waypoint: &Point, v: i32) {
    // match heading {
    //     Action::N(_) => ship.y += ship.y * v,
    //     // Action::S(_) => ship.y = (ship.y - v).abs(),
    //     Action::S(_) => ship.y -= ship.y * v,
    //     Action::E(_) => ship.x += ship.y * v,
    //     // Action::W(_) => ship.x = (ship.x - v).abs(),
    //     Action::W(_) => ship.x -= ship.x * v,
    //     _ => panic!("should only be directions here"),
    // }
